...
EventDevice::init(const std::string &filespec, size_t /* size */)
{
    ...
    switch (_device_info.bustype) {
      ...
      case BUS_I2C:
          ...
          log_unimpl(_("is an i2C bus type (InputDevice::TOUCHSCREEN)"));
          break;
      case BUS_HOST:
    ...
    if (_type == InputDevice::MOUSE || _type == InputDevice::TOUCHSCREEN) {
...
EventDevice::check()
{
    ...
    std::unique_ptr<std::uint8_t[]> buf = readData(sizeof( struct input_event) * 64, &rd);
    // time,type,code,value
    if (!buf || rd < (int) sizeof(struct input_event)) {
        return false;
    }
    ...
    static int _x = 0, _y = 0, _is_tch = 0;

    int i;
    for (i = 0; i < ev_cnt; i++) {

            if (ev[i].type == EV_ABS && (ev[i].code == ABS_X || ev[i].code == ABS_Y)) {
                if (ev[i].code == ABS_X) {
                    _x = ev[i].value;
...
//    struct input_event *ev = reinterpret_cast<struct input_event *>(buf.get());
    ...
    struct input_event *evs = reinterpret_cast<struct input_event *>(buf.get());
    for (i = 0; i < ev_cnt; i++) {
